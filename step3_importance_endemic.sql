...
drop table if exists eth_endemic_subset_anmls_endmc;
create table eth_endemic_subset_anmls_endmc as
select foo1.id_no,
...
DROP TABLE IF EXISTS out_sppimp_eth_10km_2013_anmls_endmc;
CREATE TABLE out_sppimp_eth_10km_2013_anmls_endmc AS 
...
FROM
raw.species_eoo_gridbscale AS spo,
out_spp_calc_areacells_eth_10km_2013 AS osrd,
...
-- SELECT * FROM out_sppimp_eth_10km_2013_anmls ORDER BY cell_sp LIMIT 1000;


--see how many species have values 
-- SELECT COUNT(1) from (select /*species as species*/ id_no from out_sppimp_eth_10km_2013_anmls group by id_no/*species*/) as foo;


--should be same as the count on the input table out_spp_calc_areacells_eth_10km_2013_anmls
-- SELECT count(1) from (select /*species as species*/ id_no from out_spp_calc_areacells_eth_10km_2013_anmls group by id_no/*species*/) as foo;

 
--see if null rows are null in all columns
-- SELECT * FROM out_sppimp_eth_10km_2013_anmls WHERE sppimp_eq IS NULL ORDER BY cell_sp;


--make final importance values for watersheds by grouping by watershed ids
DROP TABLE IF EXISTS out_cell_imp_eth_10km_2013_anmls_endmc;
CREATE TABLE out_cell_imp_eth_10km_2013_anmls_endmc AS
SELECT o.cell_id, SUM(o.sppimp_eq) AS cellimp_eq, SUM(o.sppimp_max) AS cellimp_max, SUM(o.sppimp_min) AS cellimp_min 
FROM out_sppimp_eth_10km_2013_anmls_endmc as o
GROUP BY o.cell_id;


--join final results to watershed polygons for viewing
DROP TABLE IF EXISTS out_cell_imp_shape_eth_10km_2013_anmls_endmc;
CREATE TABLE out_cell_imp_shape_eth_10km_2013_anmls_endmc AS 
SELECT o.*, p.the_geom as the_geom  
FROM 
out_cell_imp_eth_10km_2013_anmls_endmc AS o,
cells_eth_10km AS p 
WHERE o.cell_id=p.cell_id;

--convert to wgs84 for viewing purposes
ALTER TABLE out_cell_imp_shape_eth_10km_2013_anmls_endmc

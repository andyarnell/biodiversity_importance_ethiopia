...
drop table if exists eth_endemic_subset_plnt2_bre;
create table eth_endemic_subset_plnt2_bre as
select foo1.id_no
...
out_spp_allarearegion_eth_10km_plnt2_bre AS osrun
where
...
DROP TABLE IF EXISTS out_sppimp_eth_10km_2013_plnt2_bre;
CREATE TABLE out_sppimp_eth_10km_2013_plnt2_bre AS 
SELECT /*osrd.species,*/ osrd.id_no, osrd.cell_id, osrd.cell_sp,
CASE WHEN 
osra.sumofcell_suitarea_eq_eth_2013_plnt2_bre=0 
THEN 0 ELSE ((osrd.cell_prop/osra.sumofcell_suitarea_eq_eth_2013_plnt2_bre)*(osrun.sumofarea/spo.eoo_area)) END 
AS sppimp_eq
FROM
iucn_2014_red_list_plants_ethiopia_prj_eoo_area AS spo,
species_overlap_eth_10km_plnt AS osrd,
out_spp_allsuitarearegion_eth_10km_2013_plnt2_bre AS osra,
...
-- SELECT * FROM out_sppimp_eth_10km_2013_plnt2_bre ORDER BY cell_sp LIMIT 1000;


--see how many species have values 
-- SELECT COUNT(1) from (select /*species as species*/ id_no from out_sppimp_eth_10km_2013_plnt2_bre group by id_no/*species*/) as foo;


--should be same as the count on the input table out_spp_calc_areacells_eth_10km_2013_plnt2_bre
-- SELECT count(1) from (select /*species as species*/ id_no from out_spp_calc_areacells_eth_10km_2013_plnt2_bre group by id_no/*species*/) as foo;

 
--see if null rows are null in all columns
-- SELECT * FROM out_sppimp_eth_10km_2013_plnt2_bre WHERE sppimp_eq IS NULL ORDER BY cell_sp;


--make final importance values for watersheds by grouping by watershed ids
DROP TABLE IF EXISTS out_cell_imp_eth_10km_2013_plnt2_bre;
CREATE TABLE out_cell_imp_eth_10km_2013_plnt2_bre AS
SELECT o.cell_id, SUM(o.sppimp_eq) AS cellimp_eq--, SUM(o.sppimp_max) AS cellimp_max, SUM(o.sppimp_min) AS cellimp_min 
FROM out_sppimp_eth_10km_2013_plnt2_bre as o
GROUP BY o.cell_id;
...
DROP TABLE IF EXISTS out_cell_imp_shape_eth_10km_2013_plnt2_bre;
CREATE TABLE out_cell_imp_shape_eth_10km_2013_plnt2_bre AS 
SELECT o.*, p.the_geom as the_geom  
FROM 
out_cell_imp_eth_10km_2013_plnt2_bre AS o,
cells_eth_10km AS p 
WHERE o.cell_id=p.cell_id;

--convert to wgs84 for viewing purposes
ALTER TABLE out_cell_imp_shape_eth_10km_2013_plnt2_bre
...
FROM (select *, 1 as num from out_sppimp_eth_10km_2013_plnt2_bre) as o
GROUP BY o.num;

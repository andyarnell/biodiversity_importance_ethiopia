...
DROP TABLE IF EXISTS out_sppimp_eth_10km_2013_anmls_endmc_bre;
CREATE TABLE out_sppimp_eth_10km_2013_anmls_endmc_bre AS 
...
FROM
raw.species_eoo_gridbscale AS spo,
out_spp_calc_areacells_eth_10km_2013_bre AS osrd,
...
-- SELECT COUNT(1) from (select /*species as species*/ id_no from out_sppimp_eth_10km_2013_anmls_endmc_bre group by id_no/*species*/) as foo;


--should be same as the count on the input table out_spp_calc_areacells_eth_10km_2013_anmls_endmc_bre
-- SELECT count(1) from (select /*species as species*/ id_no from out_spp_calc_areacells_eth_10km_2013_anmls_endmc_bre group by id_no/*species*/) as foo;

 
--see if null rows are null in all columns
-- SELECT * FROM out_sppimp_eth_10km_2013_anmls_endmc_bre WHERE sppimp_eq IS NULL ORDER BY cell_sp;
...
DROP TABLE IF EXISTS out_cell_imp_eth_10km_2013_anmls_endmc_bre;
CREATE TABLE out_cell_imp_eth_10km_2013_anmls_endmc_bre AS
SELECT o.cell_id, SUM(o.sppimp_eq) AS cellimp_eq, SUM(o.sppimp_max) AS cellimp_max, SUM(o.sppimp_min) AS cellimp_min 
FROM out_sppimp_eth_10km_2013_anmls_endmc_bre as o
GROUP BY o.cell_id;


--join final results to watershed polygons for viewing
DROP TABLE IF EXISTS out_cell_imp_shape_eth_10km_2013_anmls_endmc_bre;
CREATE TABLE out_cell_imp_shape_eth_10km_2013_anmls_endmc_bre AS 
SELECT o.*, p.the_geom as the_geom  
FROM 
out_cell_imp_eth_10km_2013_anmls_endmc_bre AS o,
cells_eth_10km AS p 
WHERE o.cell_id=p.cell_id;

--convert to wgs84 for viewing purposes
ALTER TABLE out_cell_imp_shape_eth_10km_2013_anmls_endmc_bre
...
FROM (select *, 1 as num from out_sppimp_eth_10km_2013_anmls_endmc_bre) as o
GROUP BY o.num;

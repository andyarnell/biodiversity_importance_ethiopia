...
DROP TABLE IF EXISTS out_spp_allsuitareacells_eth_10km_2013_for;
CREATE TABLE out_spp_allsuitareacells_eth_10km_2013_for AS 
SELECT
h.taxonid as id_no,
...
--add column 
ALTER TABLE out_spp_allsuitareacells_eth_10km_2013_for 
ADD cell_sp varchar;
--add unique id for combination of cell and species
UPDATE out_spp_allsuitareacells_eth_10km_2013_for
SET 
...
ALTER TABLE out_spp_allsuitareacells_eth_10km_2013_for
ADD COLUMN id bigserial NOT NULL,
ADD constraint out_spp_allsuitareacells_eth_10km_2013_for_pkey PRIMARY KEY (id);



--view subset of result to check
-- SELECT * FROM out_spp_allsuitareacells_eth_10km_2013_for LIMIT 1000; 
/*SELECT os.*, (os.sumofarea_lc_cell/c.cell_area) as prop, c.cell_area 
FROM (select * from out_spp_allsuitareacells_eth_10km_2013_for) os, cells_eth_10km as c  
...
DROP TABLE IF EXISTS  out_spp_calc_areacells_eth_10km_2013_for;
CREATE TABLE out_spp_calc_areacells_eth_10km_2013_for AS
select 
/*sp.species, */
sp.id_no,
sp.cell_sp, 
...
INNER JOIN 
out_spp_allsuitareacells_eth_10km_2013_for as op 
...
ALTER TABLE out_spp_calc_areacells_eth_10km_2013_for
ADD COLUMN id bigserial NOT NULL,
ADD constraint out_spp_calc_areacells_eth_10km_2013_for_pkey PRIMARY KEY (id);


--check how many remain
-- SELECT count(1) from (select species from out_spp_calc_areacells_eth_10km_2013_for group by species) as foo;

--view subset of result to check it worked - 
-- SELECT * FROM out_spp_calc_areacells_eth_10km_2013_for LIMIT 1000;
...
FROM out_spp_calc_areacells_eth_10km_2013_for AS o
